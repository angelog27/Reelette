...
#Fetches the primary streaming service for a movie, if available. Used as a helper for concurrent lookups in fetch_movies_with_streaming.
def _fetch_streaming_for_movie(movie_data):
    try:
        providers = get_streaming_providers(movie_data['id'])
        if not providers:
            return ''
...
def fetch_movies_with_streaming(movies_data):
    # Kick off streaming lookups concurrently
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = {executor.submit(_fetch_streaming_for_movie, m): m for m in movies_data}
        service_map = {}
...
def add_user_watched(user_id):
    ...
    return jsonify(add_watched_movie(user_id, movie, user_rating))
...
def update_user_watched_rating(user_id, movie_id):
    ...
    return jsonify(update_watched_rating(user_id, movie_id, new_rating))
